...
class PokePaginate extends Component {
    ...
    render()
    {
        if (this.state.pokeList !== undefined && this.props.match.params.page >= 0 && this.props.match.params.page <49)
        {
            let Back = '';
            let Next = '';
            ...
            if (this.state.pokeList.next) {
                ...
                Next = <a class="btn btn-primary" href={`/pokemon/page/${parseInt(this.props.match.params.page, 10) + 1}`} role="button">Next</a>
            }
            if (this.state.pokeList.previous) {
                Back = <a class="btn btn-primary" href={`/pokemon/page/${parseInt(this.props.match.params.page, 10) - 1}` } role="button" >Back</a>
            }
                ...
                <div>
                    <ul>
                        {resList.map((value, index) => {
                            return (
                                <a key={index} href={`/pokemon/${value}`}>
                                <h3>{value}</h3>
                                    <br/>
                                </a>
                            )
